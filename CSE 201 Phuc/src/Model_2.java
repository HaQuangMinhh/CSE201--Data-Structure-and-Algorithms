...
public class Model_2 {
	...
	public class SampleProgram {
		...
		public static void main(String[] args) {
			...
			students.sort(Comparator.comparing(Student::getName).reversed().thenComparing(Student::getId)); //Dem diem
			...
			var ids = students.stream().sorted(studentComparator2).filter(s -> s.getId() > 10)
					.map(s -> s.getName() + " " + s.getId()).toArray(String[]::new);
